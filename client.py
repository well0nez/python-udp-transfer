...
class FileTransferClient:
    ...
    async def _run_receiver(self):
        ...
        await self._loop.sock_sendto(self._sock, msg, self.server_addr)
        ...
        for _ in range(3):
            await self._loop.sock_sendto(self._sock, ready_msg, self._peer_addr)
            await asyncio.sleep(0.1)
    ...
    async def _send_file_info(self):
        ...
        for retry in range(max_retries):
            ...
            await self._loop.sock_sendto(self._sock, info, self._peer_addr)
    ...
    async def _send_file(self):
        ...
        for retry in range(max_final_retries):
            ...
            await self._loop.sock_sendto(self._sock, done_msg, self._peer_addr)
    ...
    async def _send_chunk(self, chunk_id: int):
        ...
        await self._loop.sock_sendto(self._sock, msg, self._peer_addr)
    ...
    async def _handle_file_info(self, data: bytes):
        ...
        try:
            ...
            await self._loop.sock_sendto(self._sock, ack_msg, self._peer_addr)
    ...
    async def _do_hole_punch(self, start_at: float):
        ...
        while time.time() < end_time and self._running and not self._connected:
            await self._loop.sock_sendto(self._sock, punch_data, self._peer_addr)
            await asyncio.sleep(interval)
        
        if self._connected:
            logger.info("Hole punch succeeded! Sending confirmation packets...")
            ...
            logger.error("❌ Hole punch failed - aborting")
            self._running = False
